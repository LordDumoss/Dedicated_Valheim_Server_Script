...
function valheim_server_install() {
    ...
    if [ "${usefw}" == "y" ]; then
		...
		if [ "$usefw" = "n" ] &&
			[ "$disable_unmanaged_firewalls" = "y" ] &&
			is_any_firewall_active; then
			disable_all_firewalls
		else
			echo "Firewall management is disabled; existing firewall configuration will not be changed."
		fi
	fi
    # ========================================================================
...
##### Remove the selected Valheim world/server installation.
##### Backups are preserved.
####function valheim_server_remove() {
####    local remove_world="$worldname"
####    local install_dir="${valheimInstallPath}/${remove_world}"
####    local save_dir="${worldpath}/${remove_world}"
####    local service_name="valheimserver_${remove_world}.service"
####    local worldlist_tmp
####    local confirm_remove
####    local confirm_data
####
####    if [ -z "$remove_world" ]; then
####        echo "No Valheim world is currently selected."
####        return 1
####    fi
####
####    # World names are generated by this script as alphanumeric values.
####    # Refuse unexpected values before using them in paths or service names.
####    if [[ ! "$remove_world" =~ ^[[:alnum:]]+$ ]]; then
####        echo "Invalid world name: $remove_world"
####        return 1
####    fi
####
####    echo ""
####    tput setaf 1
####    echo "WARNING: This will remove the Valheim server installation for:"
####    echo "  World:       $remove_world"
####    echo "  Installation: $install_dir"
####    echo "  Save data:    $save_dir"
####    echo "  Service:      $service_name"
####    echo ""
####    echo "Backups will NOT be deleted:"
####    echo "  ${backupPath}/${remove_world}"
####    tput sgr0
####    echo ""
####
####    read -r -p "Continue with server removal? (y/n): " confirm_remove
####
####    if [[ ! "$confirm_remove" =~ ^[Yy]$ ]]; then
####        echo "Server removal cancelled."
####        return 0
####    fi
####
####    read -r -p \
####        "Permanently delete the installation and save data? (y/n): " \
####        confirm_data
####
...
####            echo "Unable to stop ${service_name}; removal cancelled."
####            return 1
####        fi
####    fi
####
...
####
####    # Remove the service from both locations used by this script.
...
####    systemctl daemon-reload
####
####    # Remove this world from the tracked world list.
####    if [ -f "$worldfilelist" ]; then
####        worldlist_tmp="$(mktemp "${worldfilelist}.XXXXXX")" || {
####            echo "Unable to create temporary world-list file."
####            return 1
####        }
####
####        if ! awk -v world="$remove_world" '
####            $0 != world { print }
...
####    fi
####
####    # Delete the server installation and save data only after confirmation.
...
####
####    if [ -e "$save_dir" ]; then
####        echo "Removing save directory: $save_dir"
####        rm -rf -- "$save_dir" || {
####            echo "Failed to remove save directory."
####            return 1
####        }
####    fi
####
####    # Clear the selected world so the next menu invocation reselects a world.
####    worldname=""
####
####    echo ""
####    tput setaf 2
####    echo "Valheim server removal completed."
...
function firewall_command_name() {
	local fwcommandname="none"
    case "$fwbeingused" in
        firewalld) fwcommandname="firewall-cmd" ;;
        ufw)       fwcommandname="ufw" ;;
        iptables)  fwcommandname="iptables" ;;
        *)         return 1 ;;
    esac
...
function confirm_firewall_action() {
    local prompt="$1"
    local answer
...
function firewall_state() {
    #local service="$1"
	local service="fwbeingused"
...
function is_selected_firewall_installed() {
    local firewall_cmd

    firewall_cmd="$(firewall_command_name 2>/dev/null)" || return 1
    command -v "$firewall_cmd" >/dev/null 2>&1
}

function is_selected_firewall_active() {
    local firewall_service

    firewall_service="$(firewall_service_name 2>/dev/null)" || return 1
    systemctl is-active --quiet "$firewall_service"
}

function is_selected_firewall_enabled() {
    local firewall_service
...
function is_admin_firewall_installed() {
    ...
    if firewall_cmd="$(firewall_command_name 2>/dev/null)" &&
       command -v "$firewall_cmd" >/dev/null 2>&1; then
        installed="y"
    else
        installed="n"
    fi

    # FIXED: Added proper string format routing and terminal text color reset (\033[0m)
...
function is_admin_firewall_enabled(){
    ...
    if ! firewall_service="$(firewall_service_name 2>/dev/null)"; then
        is_admin_firewall_enabled="unsupported"
    elif systemctl is-enabled --quiet $firewall_service; then
		if systemctl is-active --quiet $firewall_service; then
			is_admin_firewall_enabled="Running"
		else
			is_admin_firewall_enabled="Stopped"
		fi	
	elif ! is_selected_firewall_installed; then
        is_admin_firewall_enabled="Not found"
	else
		is_admin_firewall_enabled="Disabled"
    fi
...
function is_any_firewall_installed() {
    ...
    # Text buffers to separate the two categories
    local installed_list=""
    local not_installed_list=""
    local final_status="none found"

    # Evaluate UFW configuration layer
    if command -v ufw >/dev/null; then
        fwiufw=y
    ...
    # Evaluate FireWALD configuration layer
    if command -v firewall-cmd >/dev/null; then
        fwifwd=y
        installed_list="${installed_list}\n  - FireWALLD"
    else
    ...
    # Evaluate IPTables configuration layer
    ...
    # Evaluate IP6Tables configuration layer
    ...
    # Evaluate EBTables configuration layer
    ...
    # Set the overall macro status flag
    if [[ "$fwiufw" == "y" || "$fwifwd" == "y" || "$fwiipt" == "y" || "$fwiipt6" == "y" || "$fwiebt" == "y" ]]; then
        final_status="yes"
    fi

    # 1. Output the main macro response first
    echo -e "$final_status"
    
    # 2. Output the split layout details
    echo -e "\nInstalled Systems:${installed_list:-None}"
    # echo -e "\nNot Installed Systems:${not_installed_list:-None}\n"
}

# Report whether any supported firewall service is enabled.
function is_any_firewall_enabled() {
    # Text buffers to separate categories and global macro flag
    local enabled_list=""
    local disabled_list=""
    local final_status="no"

    # Evaluate ARPTables operational layer
    if command -v arptables >/dev/null && systemctl is-enabled arptables >/dev/null 2>&1 && systemctl is-active arptables >/dev/null 2>&1; then
        enabled_list="${enabled_list:+$enabled_list, }arptables"
    else
        disabled_list="${disabled_list:+$disabled_list, }arptables"
    fi

    # Evaluate EBTables operational layer
    if command -v ebtables >/dev/null && systemctl is-enabled ebtables >/dev/null 2>&1 && systemctl is-active ebtables >/dev/null 2>&1; then
        enabled_list="${enabled_list:+$enabled_list, }ebtables"
    else
        disabled_list="${disabled_list:+$disabled_list, }ebtables"
    fi

    # Evaluate FireWallD operational layer
    if command -v firewall-cmd >/dev/null && systemctl is-enabled firewalld >/dev/null 2>&1 && systemctl is-active firewalld >/dev/null 2>&1; then
        enabled_list="${enabled_list:+$enabled_list, }firewalld"
    else
        disabled_list="${disabled_list:+$disabled_list, }firewalld"
    fi

    # Evaluate IPTables operational layer
    if command -v iptables >/dev/null && systemctl is-enabled iptables >/dev/null 2>&1 && systemctl is-active iptables >/dev/null 2>&1; then
        enabled_list="${enabled_list:+$enabled_list, }iptables"
    else
        disabled_list="${disabled_list:+$disabled_list, }iptables"
    fi

    # Evaluate IP6Tables operational layer
    if command -v ip6tables >/dev/null && systemctl is-enabled ip6tables >/dev/null 2>&1 && systemctl is-active ip6tables >/dev/null 2>&1; then
        enabled_list="${enabled_list:+$enabled_list, }ip6tables"
    else
        disabled_list="${disabled_list:+$disabled_list, }ip6tables"
    fi

    # Evaluate UFW operational layer
    if command -v ufw >/dev/null && systemctl is-enabled ufw >/dev/null 2>&1 && systemctl is-active ufw >/dev/null 2>&1; then
        enabled_list="${enabled_list:+$enabled_list, }ufw"
    else
        disabled_list="${disabled_list:+$disabled_list, }ufw"
    fi

    # Set the overall macro enablement flag based on the text buffer
    if [[ -n "$enabled_list" ]]; then
        final_status="yes"
		echo -e "Started: (${enabled_list:-None})"
	else
		echo -e "None Enabled or Started"
    fi

    # Option B: Output compact single-line layouts
    #echo -e "$final_status Enabled and Started: (${enabled_list:-None})"
    #echo -e "Disabled/Stopped: (${disabled_list:-None})"	
}
...
function get_firewall_status() {
    ...
    if [ "$usefw" != "y" ]; then
        printf '\033[32mNoFWAdmin\n'
        return 0
    fi

    if ! firewall_service="$(firewall_service_name 2>/dev/null)"; then
        printf '\033[32mUnsupported\n'
        return 1
    fi

    if ! is_selected_firewall_installed; then
        printf '\033[32mNotInstalled\n'
        return 1
    fi

    if ! systemctl is-enabled --quiet "$firewall_service"; then
        printf '\033[32mNotEnabled\n'
        return 0
    fi

    printf '\033[32m%s\n' \
        "$(systemctl is-active "$firewall_service" 2>/dev/null || printf '%s' "inactive")"
}


# Return the systemd substate of the selected firewall.
function get_firewall_substate(){
    local firewall_service substate
    ...
    firewall_service="$(firewall_service_name 2>/dev/null)" || {
        printf '\033[32mUnsupported\n'
        return 1
    }
    ...
    substate="$(systemctl show -p SubState --value "$firewall_service" 2>/dev/null)"
    printf '\033[32m%s\n' "${substate:-unknown}"
}
...
function enable_prefered_firewall(){
    echo "Attempting to initialize active firewall system: [${fwbeingused}]..."

    # Check if the firewall binary is actually present on the OS path first
    if ! command -v "${fwbeingused}" >/dev/null 2>&1; then
        echo "Error: The binary for '${fwbeingused}' was not found on this system path."
        echo "Please verify installation before attempting to cycle the daemon state."
        return 1
    fi

    # Unmask, enable on boot, and start the target firewall process natively
    sudo systemctl unmask "${fwbeingused}" 2>/dev/null
    sudo systemctl enable "${fwbeingused}" 2>/dev/null
    sudo systemctl start "${fwbeingused}"
    
    if [ $? -eq 0 ]; then
        echo "Success: ${fwbeingused} service has been cleanly enabled and started."
    else
        echo "Failure: Systemd encountered a block while trying to initialize ${fwbeingused}."
    fi
    sleep 2
}

# FUNCTION BEGIN: stop_preferred_firewall
# ========================================================================
function stop_preferred_firewall() {
    echo "Attempting to safely terminate firewall service: [${fwbeingused}]..."

    # Gracefully handle the check if the firewall daemon doesn't exist
    if ! command -v "${fwbeingused}" >/dev/null 2>&1; then
        echo "Notice: '${fwbeingused}' binary is missing or not installed. No active service loop to halt."
        return 0
    fi

    # Immediately stop the engine and take it out of the startup boot configuration
    sudo systemctl stop "${fwbeingused}"
    sudo systemctl disable "${fwbeingused}" 2>/dev/null
    
    if [ $? -eq 0 ]; then
        echo "Success: ${fwbeingused} service has been completely stopped and disabled."
    else
        echo "Warning: Systemd generated a failure flag while attempting to halt ${fwbeingused}."
    fi
}
        ...
        if systemctl is-active --quiet "$firewall_service" ||
           systemctl is-enabled --quiet "$firewall_service"; then
            ...
            sudo systemctl disable --now \
                "$firewall_service" 2>/dev/null || true
        fi
    done

    printf '\033[32m%s\n' \
        "All known firewall systems disabled."
    sleep 2
}

# Check whether the current Valheim port range is already allowed.
function is_port_added_firewall() {
    # Accept a specific port rule passed as an argument, or fall back to Valheim ports
    local check_rule="$1"
    local target_port="${currentPort:-$portnumber}"
    [ -z "$target_port" ] && target_port=2456
    local port_max=$((target_port + 2))

    # If no rule argument was provided, format it to match your firewall syntax
    if [ -z "$check_rule" ]; then
        if [ "${fwbeingused}" == "firewalld" ]; then
            check_rule="${target_port}-${port_max}/udp"
        else
            check_rule="${target_port}/udp"
        fi
    fi
    ...
    if command -v "${fwbeingused}" >/dev/null 2>&1; then
        if [ "${fwbeingused}" == "firewalld" ]; then
            local portlistarray
            portlistarray=$(sudo firewall-cmd --zone=public --permanent --list-ports 2>/dev/null)
            if [[ "$portlistarray" == *"$check_rule"* ]]; then
                echo "y"
            else
                echo "n"
            fi
        elif [ "${fwbeingused}" == "ufw" ]; then
            # Format UFW check rule to deal with colons vs dashes in syntax
            local ufw_check
            ufw_check=$(echo "$check_rule" | tr '-' ':')
            if sudo ufw status 2>/dev/null | grep -q "$ufw_check"; then
                echo "y"
            else
                echo "n"
            fi
...
#Is the firewall service added?
function is_public_service_added() {
    local target_service="$1"
    
    if command -v "${fwbeingused}" >/dev/null 2>&1; then
        if [ "${fwbeingused}" == "firewalld" ]; then
            local servicelistarray
            servicelistarray=$(sudo firewall-cmd --zone=public --permanent --list-services 2>/dev/null)
            
            # Check if the service name exists as a whole word boundary
            if [[ " $servicelistarray " == *" ${target_service} "* ]]; then
                echo "y"
            else
                echo "n"
            fi
        elif [ "${fwbeingused}" == "ufw" ]; then
            # Check if the application profile is currently allowed in UFW status
            if sudo ufw status 2>/dev/null | grep -q "${target_service}"; then
                echo "y"
            else
                echo "n"
            fi
...
function add_Valheim_server_public_ports() {
    if [ "${usefw}" == "y" ]; then
        ...
        if [ "${fwbeingused}" == "firewalld" ]; then
            if [ "$sftc" == "ste" ]; then
                echo "Validating SteamCMD Rules in Firewalld..."
                local steam_ports=("1200/udp" "27000-27015/udp" "27020/udp" "27015-27016/tcp" "27030-27039/tcp")

                for port_rule in "${steam_ports[@]}"; do
                    if [ "$(is_port_added_firewall "$port_rule")" == "y" ]; then
                        echo "  -> Skip: Firewalld already contains rule [$port_rule]"
                    else
                        echo "  -> Adding: Injecting rule [$port_rule] into Firewalld zone..."
                        sudo firewall-cmd --zone=public --permanent --add-port="$port_rule" >/dev/null
                        change_made="y"
                    fi
                done
                
            elif [ "$sftc" == "val" ]; then
                local val_rule="${target_port}-${port_max}/udp"
                echo "Validating Valheim [${worldname}] Rules in Firewalld..."
                
                if [ "$(is_port_added_firewall "$val_rule")" == "y" ]; then
                    echo "  -> Skip: Firewalld already contains rule [$val_rule]"
                else
                    echo "  -> Adding: Injecting rule [$val_rule] into Firewalld zone..."
                    sudo firewall-cmd --zone=public --permanent --add-port="$val_rule" >/dev/null
                    change_made="y"
                fi
            fi

            if [ "$change_made" == "y" ]; then
                sudo firewall-cmd --reload >/dev/null
                echo "Firewall changes committed successfully."
            fi
            ...
            if [ "$sftc" == "ste" ]; then
                echo "Validating SteamCMD Rules in UFW Layout..."
                local ufw_steam=("1200/udp" "27020/udp" "27000:27015/udp" "27015:27016/tcp" "27030:27039/tcp")

                for port_rule in "${ufw_steam[@]}"; do
                    if [ "$(is_port_added_firewall "$port_rule")" == "y" ]; then
                        echo "  -> Skip: UFW already contains rule [$port_rule]"
                    else
                        echo "  -> Adding: Injecting rule [$port_rule] into UFW rules layout..."
                        sudo ufw allow "$port_rule" >/dev/null
                        change_made="y"
                    fi
                done

            elif [ "$sftc" == "val" ]; then
                local val_ufw_rule="${target_port}:${port_max}/udp"
                echo "Validating Valheim [${worldname}] Rules in UFW Layout..."

                if [ "$(is_port_added_firewall "$val_ufw_rule")" == "y" ]; then
                    echo "  -> Skip: UFW already contains rule [$val_ufw_rule]"
                else
                    echo "  -> Adding: Injecting rule [$val_ufw_rule] into UFW rules layout..."
                    sudo ufw allow "$val_ufw_rule" >/dev/null
                    change_made="y"
                fi
            fi
            ...
            if [ "$change_made" == "y" ]; then
                ...
                echo "UFW layout cleanly reloaded."
            fi
...
function remove_Valheim_server_public_ports() {
    if [ "${usefw}" == "y" ]; then
        ...
        if ! confirm_firewall_action \
            "Remove the Valheim port range from the firewall?"; then
        ...
        if [ "${fwbeingused}" == "firewalld" ]; then
            if [ "$sftc" == "ste" ]; then
                echo "Validating SteamCMD Rules in Firewalld before purge..."
                local steam_ports=("1200/udp" "27000-27015/udp" "27020/udp" "27015-27016/tcp" "27030-27039/tcp")

                for port_rule in "${steam_ports[@]}"; do
                    # Use our helper function to see if the rule is actually there
                    if [ "$(is_port_added_firewall "$port_rule")" == "y" ]; then
                        echo "  -> Removing: Purging rule [$port_rule] from Firewalld zone..."
                        sudo firewall-cmd --zone=public --permanent --remove-port="$port_rule" >/dev/null
                        change_made="y"
                    else
                        echo "  -> Skip: Rule [$port_rule] is already missing from Firewalld."
                    fi
                done
                
            elif [ "$sftc" == "val" ]; then
                local val_rule="${target_port}-${port_max}/udp"
                echo "Validating Valheim [${worldname}] Rules in Firewalld before purge..."
                
                if [ "$(is_port_added_firewall "$val_rule")" == "y" ]; then
                    echo "  -> Removing: Purging rule [$val_rule] from Firewalld zone..."
                    sudo firewall-cmd --zone=public --permanent --remove-port="$val_rule" >/dev/null
                    change_made="y"
                else
                    echo "  -> Skip: Rule [$val_rule] is already missing from Firewalld."
                fi
            fi

            # Only reload Firewalld if something was actually removed
            if [ "$change_made" == "y" ]; then
                sudo firewall-cmd --reload >/dev/null
                echo "Firewall changes committed successfully."
            fi
            ...
            if [ "$sftc" == "ste" ]; then
                echo "Validating SteamCMD Rules in UFW before purge..."
                local ufw_steam=("1200/udp" "27020/udp" "27000:27015/udp" "27015:27016/tcp" "27030:27039/tcp")

                for port_rule in "${ufw_steam[@]}"; do
                    if [ "$(is_port_added_firewall "$port_rule")" == "y" ]; then
                        echo "  -> Removing: Deleting rule [$port_rule] from UFW layout..."
                        sudo ufw delete allow "$port_rule" >/dev/null
                        change_made="y"
                    else
                        echo "  -> Skip: Rule [$port_rule] is already missing from UFW."
                    fi
                done

            elif [ "$sftc" == "val" ]; then
                local val_ufw_rule="${target_port}:${port_max}/udp"
                echo "Validating Valheim [${worldname}] Rules in UFW before purge..."

                if [ "$(is_port_added_firewall "$val_ufw_rule")" == "y" ]; then
                    echo "  -> Removing: Deleting rule [$val_ufw_rule] from UFW layout..."
                    sudo ufw delete allow "$val_ufw_rule" >/dev/null
                    change_made="y"
                else
                    echo "  -> Skip: Rule [$val_ufw_rule] is already missing from UFW."
                fi
            fi

            # Only reload UFW if changes occurred
            if [ "$change_made" == "y" ]; then
                sudo ufw reload >/dev/null 2>&1
                echo "UFW layout cleanly reloaded."
            fi
...
### 6. Do not add a Firewalld service before verifying its XML file exists
### ...The same validation should be added to remove_firewalld_public_service function.
###
### function add_firewalld_public_service() {
...
###             ;;
###         val)
###             service_name="valheimserver-${worldname}"
###             ;;
###         *)
###             echo "Unknown Firewalld service type: $sftc"
###             return 1
###             ;;
###     esac
### 
###     service_file="/etc/firewalld/services/${service_name}.xml"
### 
###     if [ ! -f "$service_file" ]; then
###         echo "Service definition does not exist:"
###         echo "  $service_file"
###         echo "Create it before adding it to the public zone."
###         return 1
###     fi
### 
###     if ! sudo firewall-cmd --zone=public --permanent \
###         --add-service="$service_name"; then
###         echo "Unable to add Firewalld service: $service_name"
###         return 1
###     fi
### 
###     sudo firewall-cmd --reload
...
export LD_LIBRARY_PATH=./linux64:\$LD_LIBRARY_PATH
export SteamAppId=892970
./valheim_server.x86_64 -name "${setCurrentDisplayName}" -port "${setCurrentPort}" -nographics -batchmode -world "${setCurrentWorldName}" -password "${setCurrentPassword}" -public "${setCurrentPublicSet}" -crossplay "${setCurrentCrossplayStatus}" -savedir "${setCurrentSaveDir}" -logfile "${setCurrentLogfileDir}" 
export LD_LIBRARY_PATH=\$templdpath
EOF
...
function are_mods_enabled() {
    ...
    var2="ExecStart=${valheimInstallPath}/$remove_firewalld_public_serviceaddadadad{worldname}/start_server_bepinex.sh"
...
function firewall_admin_menu() {
    ...
    if [ "${usefw}" != "y" ]; then
        ...
        echo "Set usefw=\"y\" in the script configuration to enable firewall controls."
    ...
    local firewall_cmd=""
    firewall_cmd="$(firewall_command_name 2>/dev/null || true)"

    echo -ne "
$(ColorPurple '╔══════════════════════════════════════════════════════════╗')
$(ColorPurple '║~~~~~~~~~~*****~~~~~~~~-Njord Menu-~~~~~~~~~*****~~~~~~~~~║')
$(ColorPurple '╠══════════════════════════════════════════════════════════╣')
$(ColorPurple '║~~Valheim Server Firewall Information and Control Center~~║')
$(ColorPurple '╠══════════════════════════════════════════════════════════╝')
$(ColorPurple '║') Configured firewall: $(ColorGreen "${fwbeingused}")
$(ColorPurple '║') Firewall command:    $(ColorGreen "${firewall_cmd:-unsupported}")
$(ColorPurple '╠═══════════════════════════════════════════════════════════')
$(ColorPurple '║')" $FUNCTION_HEADER_MENU_INFO_LD_SEVER_SESSION $(ColorGreen ''"${worldname}"'')
	echo -ne "
$(ColorPurple '║')" $FUNCTION_HEADER_MENU_INFO_SERVER_NAME $(ColorGreen ''"${currentDisplayName}"'')
	echo -ne "
...
$(ColorPurple '║') $FUNCTION_HEADER_MENU_INFO_SERVER_PORT" $(ColorGreen ''"${currentPort}"'')
	echo -ne "
$(ColorPurple '║') $FUNCTION_HEADER_MENU_INFO_PUBLIC_LIST" $(ColorGreen ''"$(display_public_status_on_or_off)"'')
	echo -ne "
$(ColorPurple '║') $FUNCTION_HEADER_MENU_INFO_SERVER_AT_GLANCE" $(server_status) and $(server_substate)
	echo -ne "
$(ColorPurple '║') Configured installed: $(ColorLightGreen "$(is_admin_firewall_installed)")
$(ColorPurple '║') Configured enabled:   $(ColorLightGreen "$(is_admin_firewall_enabled)")
$(ColorPurple '║') Any installed:        $(ColorLightGreen "$(is_any_firewall_installed)")
$(ColorPurple '║') Any enabled:          $(ColorLightGreen "$(is_any_firewall_enabled)")
$(ColorPurple '╠═══════════════════════════════════════════════════════════')
$(ColorPurple '║')$(ColorLightGreen ' Firewall status and information')
$(ColorPurple '╠═══════════════════════════════════════════════════════════')
$(ColorPurple '║ ')$(ColorGreen '1)') Show selected firewall status
$(ColorPurple '║ ')$(ColorGreen '2)') Show selected firewall substate
$(ColorPurple '║ ')$(ColorGreen '3)') Show detailed firewall information
$(ColorPurple '║ ')$(ColorGreen '4)') Check Valheim port rule status
$(ColorPurple '║ ')$(ColorGreen '5)') Show current Firewalld/UFW rules
$(ColorPurple '╠═══════════════════════════════════════════════════════════')
$(ColorPurple '║')$(ColorLightGreen ' Port rule management')
$(ColorPurple '╠═══════════════════════════════════════════════════════════')
$(ColorPurple '║ ')$(ColorGreen '6)') Vaildate SteamCMD ports
$(ColorPurple '║ ')$(ColorGreen '7)') Add SteamCMD ports
$(ColorPurple '║ ')$(ColorGreen '8)') Remove SteamCMD ports
$(ColorPurple '║ ')$(ColorGreen '9)') Validate this Valheim server port
$(ColorPurple '║ ')$(ColorGreen '10)') Add this Valheim server port
$(ColorPurple '║ ')$(ColorGreen '11)') Remove this Valheim server port"

    if [ "${fwbeingused}" = "firewalld" ]; then
        echo -ne "
$(ColorPurple '╠═══════════════════════════════════════════════════════════')
$(ColorPurple '║')$(ColorLightGreen ' Firewalld service-profile management')
$(ColorPurple '╠═══════════════════════════════════════════════════════════')
$(ColorPurple '║ ')$(ColorGreen '50)') Create Valheim service profile
$(ColorPurple '║ ')$(ColorGreen '51)') Delete Valheim service profile
$(ColorPurple '║ ')$(ColorGreen '52)') Add Valheim service to public zone
$(ColorPurple '║ ')$(ColorGreen '53)') Remove Valheim service from public zone"
    fi
    echo -ne "
$(ColorPurple '╠═══════════════════════════════════════════════════════════')
$(ColorPurple '║')$(ColorLightGreen ' Firewall administration')
$(ColorPurple '╠═══════════════════════════════════════════════════════════')
$(ColorPurple '║ ')$(ColorGreen '100)') Check preferred firewall installation
$(ColorPurple '║ ')$(ColorGreen '101)') Enable and start preferred firewall
$(ColorPurple '║ ')$(ColorGreen '102)') Stop and disable preferred firewall
$(ColorPurple '║ ')$(ColorGreen '103)') Stop and disable all known firewalls
$(ColorPurple '╠═══════════════════════════════════════════════════════════')
$(ColorPurple '║ ')$(ColorGreen '0)') Return to main menu
$(ColorPurple '╚═══════════════════════════════════════════════════════════')
$(ColorOrange ''"$DRAW60"'')
$(ColorPurple ''"$CHOOSE_MENU_OPTION"'') "
    ...
    case "$a" in
        ...
        4)
            currentPort="${currentPort:-$portnumber}"
            is_port_added_firewall
            echo "Press Enter to continue..."
            read -r
            firewall_admin_menu
            ;;
        ...
        5)
            if [ "${fwbeingused}" = "firewalld" ]; then
                ...
                echo "Rule display is not implemented for: ${fwbeingused}"
            fi
        ...
        50)
            if [ "${fwbeingused}" != "firewalld" ]; then
                echo "This option requires Firewalld."
                sleep 2
            else
                sftc="val"
        ...
        101)
            enable_prefered_firewall
            firewall_admin_menu
            ;;

        102)
            stop_preferred_firewall
            firewall_admin_menu
            ;;
		...
		103)
            echo ""
            tput setaf 1
            echo "WARNING: This will stop and disable all known firewall services."
            echo "This may expose the server and may affect other applications."
            tput sgr0
            echo ""

            read -r -p "Continue? (y/n): " confirmDisableFirewalls

            if [[ "$confirmDisableFirewalls" =~ ^[Yy]$ ]]; then
                disable_all_firewalls
            else
                echo "Firewall disable operation cancelled."
                sleep 2
            fi
